...
export function RegisterForm({ onSwitchView }: RegisterFormProps) {
  ...
  return <div className="flex flex-col gap-4">
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      ...
      <Button type="submit" disabled={isLoading} className="group mt-1 h-11 w-full rounded-xl bg-foreground text-sm font-semibold text-background shadow-[0_12px_24px_-10px_rgba(0,0,0,0.55)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-foreground/90 hover:shadow-[0_18px_30px_-10px_rgba(0,0,0,0.65)] active:translate-y-0">{isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Criar minha conta <ArrowRight className="ml-2 h-4 w-4 transition-transform duration-300 group-hover:translate-x-1.5" /></>}</Button>
    </form>
    <p className="pt-0.5 text-center text-xs text-muted-foreground">Já tem uma conta? <button type="button" onClick={() => onSwitchView("login")} className="font-semibold text-foreground transition-colors hover:text-foreground/70">Fazer login</button></p>
  </div>;
}
